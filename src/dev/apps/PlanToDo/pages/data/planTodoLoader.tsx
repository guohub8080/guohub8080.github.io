/**
 * PlanTodo 动态加载器
 * 自动扫描 src/dev/apps/PlanTodo/pages 文件夹下的所有 .tsx, .md, .mdx 文件
 * 按文件夹分组，每个文件夹对应一个分类
 * 
 * 文件夹命名规则：[x]分类名
 * - [x] - 排序号，数字越小越靠上
...
import ArticleContent from '../../../../components/layout/BookLayout/ArticleContent.tsx';
import { BookLayoutConfigProvider } from '../../../../components/layout/BookLayout/internal/BookLayoutContext.tsx';
import planTodoConfig from './info.tsx';
import type { LucideIcon } from "lucide-react";
import type { BookLoader, BookCategory, BookArticle } from '../../../../components/layout/BookLayout/types/BookLoader.ts';
import { globalSettingsStore } from '../../../../store/useGlobalSettings';
import { MDXProviderWrapper, detectFileType, processDocumentContent } from '../../../../components/mdx/index.ts';
...
// 动态导入 pages 文件夹下的所有文件（排除 data 目录）
// 同时导入默认导出和命名导出（如 export const title）
...
function groupAndSortDocuments(): DocumentCategory[] {
  ...
  Object.entries(documentModules).forEach(([path, module]) => {
    ...
    // 处理文档内容，优先使用命名导出的 title
    let { title, jsxContent } = processDocumentContent(document, fileType, fileName);
    
    // 如果模块有命名导出的 title，优先使用它（MDX 文件的 export const title）
    if (module.title) {
      title = module.title;
    }
    ...
    const fullUrl = `${planTodoConfig.slug}/${routePath}`;
...
export const planTodoCategories = groupAndSortDocuments();
export const allPlanTodoDocuments = planTodoCategories.flatMap(cat => cat.articles);

const planTodoLoader: BookLoader = {
  config: {
    title: planTodoConfig.title,
    slug: planTodoConfig.slug,
    icon: () => planTodoConfig.icon,
    description: planTodoConfig.description
  },
  categories: planTodoCategories,
  getAllArticles(): BookArticle[] { return allPlanTodoDocuments; },
  getCurrentArticle(path: string): BookArticle | null {
    const normalizedPath = path.toLowerCase().replace(/^\/+|\/+$/g, '');
    return allPlanTodoDocuments.find(a => a.routePath.toLowerCase() === normalizedPath) || null;
  },
  getNavigationData() { return { categories: planTodoCategories, allArticles: allPlanTodoDocuments }; },
  initializeBookState() { globalSettingsStore.getState().setIsBookPage(true); }
};

export function generatePlanTodoRoutes(): RouteObject[] {
  return [
    {
      path: planTodoConfig.slug,
      element: (
        <BookLayoutConfigProvider basePrefix={`/${planTodoConfig.slug}`}>
          <BookLayout loader={planTodoLoader} />
        </BookLayoutConfigProvider>
      ),
      children: [
        {
          index: true,
          element: allPlanTodoDocuments.length > 0
            ? (<Navigate to={allPlanTodoDocuments[0].routePath} replace />)
            : <div className="p-6 text-center text-muted-foreground">暂无文章</div>,
        },
        ...planTodoCategories.flatMap((category) =>
          category.articles.map((article) => ({
            path: article.routePath,
            element: <ArticleContent loader={planTodoLoader}>{article.content}</ArticleContent>,
          }))
        ),
      ],
    },
