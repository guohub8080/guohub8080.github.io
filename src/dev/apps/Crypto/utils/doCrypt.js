...
const encrypt_string_by_b64_public_key = (plain_text, public_b64_key) => {
    const encrypt = new JSEncrypt();
    encrypt.setPublicKey(Base64.decode(public_b64_key));
    return encrypt.encrypt(plain_text);
}

const decrypt_string_by_b64_private_key = (encrypted_text, private_b64_key) => {
    const decrypt = new JSEncrypt();
    decrypt.setPrivateKey(Base64.decode(private_b64_key));
    return decrypt.decrypt(encrypted_text);
}

const decrypt_json_by_b64_private_key = (encrypted_json, private_b64_key) => {
    return JSON.parse(decrypt_string_by_b64_private_key(encrypted_json, private_b64_key) )
}

const encrypt_obj_by_b64_public_key = (base_obj, public_b64_key) => {
