/**
 * IME 动态加载器
 * 自动扫描 src/dev/apps/IME/pages 文件夹下的所有 .tsx, .md, .mdx 文件
 * 按文件夹分组，每个文件夹对应一个分类
 * 
 * 文件夹命名规则：[x]分类名
 * - [x] - 排序号，数字越小越靠上
...
import imeConfig from './info.tsx';
...
function groupAndSortDocuments(): DocumentCategory[] {
  ...
  Object.entries(documentModules).forEach(([path, module]) => {
    ...
    const fullUrl = `${imeConfig.slug}/${routePath}`;
...
export const imeCategories = groupAndSortDocuments();
export const allIMEDocuments = imeCategories.flatMap(cat => cat.articles);

const imeLoader: BookLoader = {
  config: {
    title: imeConfig.title,
    slug: imeConfig.slug,
    icon: () => imeConfig.icon,
    description: imeConfig.description
  },
  categories: imeCategories,
  getAllArticles(): BookArticle[] { return allIMEDocuments; },
  getCurrentArticle(path: string): BookArticle | null {
    const normalizedPath = path.toLowerCase().replace(/^\/+|\/+$/g, '');
    return allIMEDocuments.find(a => a.routePath.toLowerCase() === normalizedPath) || null;
  },
  getNavigationData() { return { categories: imeCategories, allArticles: allIMEDocuments }; },
  initializeBookState() { globalSettingsStore.getState().setIsBookPage(true); }
};

export function generateIMERoutes(): RouteObject[] {
  return [
    {
      path: imeConfig.slug,
      element: (
        <BookLayoutConfigProvider basePrefix={`/${imeConfig.slug}`}>
          <BookLayout loader={imeLoader} />
        </BookLayoutConfigProvider>
      ),
      children: [
        {
          index: true,
          element: allIMEDocuments.length > 0
            ? (<Navigate to={allIMEDocuments[0].routePath} replace />)
            : <div className="p-6 text-center text-muted-foreground">暂无文章</div>,
        },
        ...imeCategories.flatMap((category) =>
          category.articles.map((article) => ({
            path: article.routePath,
            element: <ArticleContent loader={imeLoader}>{article.content}</ArticleContent>,
          }))
        ),
      ],
    },
