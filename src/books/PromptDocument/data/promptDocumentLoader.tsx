...
import promptDocumentConfig from './info.tsx';
...
const documentModules = Object.fromEntries(
  Object.entries(allModules).filter(([path]) => 
    path.startsWith("../") &&          // 仅限上级目录内容（排除 ./PromptIcon.tsx 等）
    !/\.\/components\//.test(path) && // 排除本地 components
    !/\/info\.tsx$/.test(path) &&      // 排除分类 info
    !/\/data\//.test(path)             // 排除 data 目录（如 PromptIcon 等）
  )
);
...
function groupAndSortDocuments(): DocumentCategory[] {
  ...
  Object.entries(documentModules).forEach(([path, module]) => {
    ...
    const fullUrl = `${promptDocumentConfig.slug}/${routePath}`;
...
const promptDocumentLoader: BookLoader = {
  config: {
    title: promptDocumentConfig.title,
    slug: promptDocumentConfig.slug,
    icon: () => promptDocumentConfig.icon,
    description: promptDocumentConfig.description
  },
...
export function generatePromptDocumentRoutes(): RouteObject[] {
  return [
    {
      path: promptDocumentConfig.slug,
      element: (
        <BookLayoutConfigProvider basePrefix={`/${promptDocumentConfig.slug}`}>
          <BookLayout loader={promptDocumentLoader} />
      ...
      children: [
        ...
        ...userDocumentCategories.flatMap((category) =>
          category.articles.map((article) => ({
            ...
            element: <ArticleContent loader={promptDocumentLoader}>{article.content}</ArticleContent>,
          }))
        ),
      ],
    },
...
export default promptDocumentLoader;
