...
const title = "按照需求完成公文写作";

const RequestRequirement: React.FC = () => {
  const [extraRequest, setExtraRequest] = useState('');
  const [promptText, setPromptText] = useState('');

  useEffect(() => {
    const extra = extraRequest.trim() || '目前任务并不清楚，请你立刻询问我，并根据我的需求完成公文写作。';
    const template = `你是一位精通中国公文写作的专家级AI助手。你的唯一任务是接收我提供的文本内容，并严格按照以下规则将其整理成标准的公文格式。
...
const RequestRequirementPage = {
  title,
  jsx: React.createElement(RequestRequirement)
};

export default RequestRequirementPage;
