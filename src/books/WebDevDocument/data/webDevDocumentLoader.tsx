/**
 * WebDevDocument 用户指引动态加载器
 * 自动扫描 src/books/WebDevDocument 文件夹下的所有 .tsx, .md, .mdx 文件
 * 按文件夹分组，每个文件夹对应一个分类
 * 
 * 文件夹命名规则：[x]M
 * - [x] - 排序号，数字越小越靠上
 ...
 * - [x] - 排序号（数字，组内排序）
 * - slug - URL 路径（全英文，如：react-hooks, css-grid）
...
import webDevDocumentConfig from './info.tsx';
...
const documentModules = Object.fromEntries(
  Object.entries(allModules).filter(([path]) => 
    path.startsWith("../") &&          // 仅限上级目录内容（排除 ./WebDevIcon.tsx 等）
...
function groupAndSortDocuments(): DocumentCategory[] {
  ...
  Object.entries(documentModules).forEach(([path, module]) => {
    ...
    const fullUrl = `${webDevDocumentConfig.slug}/${routePath}`;
...
export const webDevDocumentCategories = groupAndSortDocuments();
export const allWebDevDocuments = webDevDocumentCategories.flatMap(cat => cat.articles);

const webDevDocumentLoader: BookLoader = {
  config: {
    title: webDevDocumentConfig.title,
    slug: webDevDocumentConfig.slug,
    icon: () => webDevDocumentConfig.icon,
    description: webDevDocumentConfig.description
  },
  categories: webDevDocumentCategories,
  getAllArticles(): BookArticle[] { return allWebDevDocuments; },
  getCurrentArticle(path: string): BookArticle | null {
    const normalizedPath = path.toLowerCase().replace(/^\/+|\/+$/g, '');
    return allWebDevDocuments.find(a => a.routePath.toLowerCase() === normalizedPath) || null;
  },
  getNavigationData() { return { categories: webDevDocumentCategories, allArticles: allWebDevDocuments }; },
  initializeBookState() { globalSettingsStore.getState().setIsBookPage(true); }
};

export function generateWebDevDocumentRoutes(): RouteObject[] {
  return [
    {
      path: webDevDocumentConfig.slug,
      element: (
        <BookLayoutConfigProvider basePrefix={`/${webDevDocumentConfig.slug}`}>
          <BookLayout loader={webDevDocumentLoader} />
        </BookLayoutConfigProvider>
      ),
      children: [
        {
          index: true,
          element: allWebDevDocuments.length > 0
            ? (<Navigate to={allWebDevDocuments[0].routePath} replace />)
            : <div className="p-6 text-center text-muted-foreground">暂无文章</div>,
        },
        ...webDevDocumentCategories.flatMap((category) =>
          category.articles.map((article) => ({
            path: article.routePath,
            element: <ArticleContent loader={webDevDocumentLoader}>{article.content}</ArticleContent>,
          }))
        ),
      ],
    },
...
export default webDevDocumentLoader;
