...
import getRandomPartName from "../../data/getRandomPartName.ts";
...
const NewAddScorePartPanel = (props: {
	switchOpen: (i: any) => void
	onOK: (i: object) => any
	onCancel?: () => void
	setObj?: (i: object) => void
}) => {
	const {fontFamily} = useGlobalSettings()
	const baseConfig = useBaseScoreConfig()
	const [partObj, setPartObj] = useState({...defaultPartObj, partName: getRandomPartName()})
		...
		<div className="innerFrame">
			...
			<div style={{width: "100%", ...cssPresets.flexCenter, marginTop: 25, gap: 20}}>
				<Button style={{width: 150}} onClick={() => {
					props.onOK({partMeta: partObj, notesList: []})
					setPartObj({...defaultPartObj, partName: getRandomPartName()})
				}} type="primary">
					{t({cn: "确定", en: "Confirm"})}
				</Button>
				<Button style={{width: 150}} onClick={props.onCancel}>
					{t({cn: "取消", en: "Cancel"})}
				</Button>
			</div>
		</div>
...
export default NewAddScorePartPanel